...
def tensor_sum(list_of_lists):   
    ...
    tensor = cp.zeros(shape)
    M = len(shape)
    idxes = list(range(len(shape)))
    for idx, l in enumerate(list_of_lists):
        l_cuda = cp.asarray(l)
        reshaped_tensor = l_cuda.reshape(*([1] * idx + [-1] + [1] * (M - idx - 1)))
        broadcasted_tensor = cp.broadcast_to(reshaped_tensor, shape)
...
def create_normed_cost_tensor(list_list_of_lists):
    ...
    return res.astype(cp.float64), cp.array([1/n for n in shape for _ in range(n)]).astype(cp.float64)
...
def marginal_k(X, k):
    return cp.sum(X, axis=tuple(axis for axis in range(X.ndim) if axis != k))
...
def projection(X, p):
    ...
    idxes = list(range(len(shape)))
    for r in range(m):
        X_r = cp.minimum(get_marginal_k(p, r, shape) / marginal_k(V, r), 1)
        V *= cp.transpose(cp.tensordot(X_r, cp.ones(shape[r+1:] + shape[:r]), axes=0), axes=rotate(idxes, -r))

    err_list = [get_marginal_k(p, r, shape) - marginal_k(V, r) for r in range(m)]
    V += reduce(cp.multiply, cp.ix_(*err_list)) / (cp.abs(err_list[-1]).sum() ** (m-1))
    return V

def rho(a, b):
    return cp.matmul(cp.exp(a), (a - b))
...
def solve_multi_sinkhorn(costs, target_mu,opt=None, epsilon=1e-2, target_epsilon=1e-4, verbose = 0, epsilon_scale_num = 1, epsilon_scale_gap = 100, cost_scale = 1, iter_gap = 100, max_iter = 5000, out_dir = 'test'):
    ...
    def dist(B):
        return cp.sum(cp.asarray([cp.sum(cp.abs(marginal_k(B, i) - get_marginal_k(target_mu, i, shape))) for i in range(M)]))
    ...
    def stable_update():
        ...
        for k in range(M):
            ...
            tmp_tensor = tmp_max.reshape(*([1]*k + [-1] + [1]*(M - k - 1)))
            tmp_tensor = cp.broadcast_to(tmp_tensor, shape)

            target_marginal_list.append(cp.log(get_marginal_k(target_mu, k, shape)))

            temp_exp = cp.exp(exponents - tmp_tensor)  # Exponential operation
            sum_result = cp.sum(temp_exp, axis=tuple(idxes))  # Sum along specified axes
            log_result = cp.log(sum_result)  # Logarithm
            gamma_marginal_result = log_result + tmp_max  # Add tmp_max
            gamma_marginal_list.append(gamma_marginal_result)

            rho_list.append(rho(target_marginal_list[k], gamma_marginal_list[k]))

        k_star = cp.argmax(cp.asarray(rho_list)).item()
    ...
    # while iter < 500:
        if iter == 0: # update first step
            ...
            tmp_tensor = tmp_max.reshape(*([1]*1 + [-1] + [1]*(M - 1 - 1)))
            tmp_tensor = cp.broadcast_to(tmp_tensor, shape)
            
            temp_exp = cp.exp(exponents - tmp_tensor)
            sum_result = cp.sum(temp_exp, axis=tuple(idxes))
            m[1] += cp.log(get_marginal_k(target_mu, 1, shape)) - cp.log(sum_result) - tmp_max
        else:
            stable_update()
...
def solve_multi_greenkhorn(costs, target_mu, opt=None ,epsilon=1e-2, target_epsilon=1e-4, verbose = 0, epsilon_scale_num = 1, epsilon_scale_gap = 100, cost_scale = 1, iter_gap = 100, max_iter = 5000, out_dir = 'test'):
    ...
    def dist(B):
        ...
        return cp.sum(cp.asarray([cp.sum(cp.abs(marginal_k(B, i) - get_marginal_k(target_mu, i, shape))) for i in range(B.ndim)]))
    start=time()        
    while True:
        max_v = []
        buffer = []
        ...
        kmax = cp.argmax(cp.asarray([max_v])).item()
...
def solve_pd_aam(costs, target_mu,opt=None , epsilon_final = 1e-6, verbose = 0, print_itr = 0, max_iterate = 50, method = "binary_search", cost_scale = 1, epsilon0 = 0.01, halflife = 1, out_dir = 'test'):
    ...
    m_gpu, n_gpu = cp.asarray(m), cp.asarray(n)

    MAX_DIGIT = 25
    C_norm = costs.max().max()
    
    p = cp.array(target_mu)
    uniform = cp.array([1/n for n in shape for _ in range(n)])
    ...
    def l1_error(X, p):
        ...
        return cp.sum(cp.asarray([cp.sum(cp.abs(marginal_k(X, k) - get_marginal_k(p, k, shape))) for k in range(X.ndim)]))

    def F(X, gamma):
        return (costs * X).sum().sum() - gamma * (X * cp.log(X)).sum().sum()
    ...
    def B_stable(U, gamma):
        ...
        U_tmp = cp.maximum(U_tmp, max_U_tmp - MAX_DIGIT)
        min_U_tmp = U_tmp.min().min()
        return cp.exp(U_tmp - min_U_tmp), min_U_tmp
    
    def log_B(U, gamma):
        exp_U_tmp, min_U_tmp = B_stable(U, gamma)
        return min_U_tmp + cp.log(exp_U_tmp.sum().sum())
    ...
    def g(theta, p, gamma):
        return cp.concatenate([g_k(k, theta, p, gamma) for k in range(m)])
    
    def g_l2_norm(theta, p, gamma):
        return cp.sum(cp.asarray([g_l2_norm_k(k, theta, p, gamma) for k in range(m)]))
    
    def g_l2_norm_k(k, theta, p, gamma):
        return cp.square(g_k(k, theta, p, gamma)).sum() # marginal_k(U_tmp, k)
    
    def update_eta(theta, i, gamma): # lemma 2 iteration step
        eta = theta.copy()
        exp_U_tmp, min_U_tmp = B_stable(eta, gamma)
        log_B_marginal = min_U_tmp + cp.log(cp.sum(exp_U_tmp, axis=tuple(axis for axis in range(exp_U_tmp.ndim) if axis != i)))
        eta_update = get_marginal_k(theta, i, shape) + cp.log(get_marginal_k(p, i, shape)) - log_B_marginal
    ...
    eta = cp.zeros(len(p))
    zeta = cp.zeros(len(p))
    theta = cp.zeros(len(p))
    x0 = 0.1

    # TODO: what's the initialization for X?
    X = cp.ones(costs.shape)
    ...
    p_tilde = (1 - epsilon_p/(4 * m_gpu)) * cp.array(target_mu) + epsilon_p / (4 * m_gpu) * uniform
    start=time()
    while 2 * l1_error(X, p_tilde) + F(X, gamma) + psi(eta, p_tilde, gamma) > epsilon / 2:
        ...
        # pick margin
        i = cp.argmax(cp.asarray([g_l2_norm_k(k, theta, p_tilde, gamma) for k in range(m)])).item()
